/*
ORIGINAL CODE BY kgolid
https://github.com/kgolid/p5ycho/tree/master/horizon4
 */

let seed;
let points;
let current;
let deviation = 100;//100
...
function draw ()
{
    ...
    for (var h = 0; h < 4; h++)//6
    {
        ...
        // let hue = random(360);
        let hue = 25;
        // fill(hue, 100, 95, 0.012);
        fill(hue, 100, 95, 0.01);
        for (var i = 0; i < 40; i++)//45
        {
            current = update();
            display_row();
        }
...
function init (ypos)
{
    let initial_size = 2;//5
    let initial_deviation = 350;//350
    let number_of_interpolations = 5;//8

    points = [];
    for (var i = 0; i < initial_size; i++)
    {
        let vec = createVector(i / (initial_size - 1) * width, ypos, random(-1, 1));
...
{
    pnt.x = randomGaussian(pnt.x, pnt.z * sd);
    pnt.y = randomGaussian(pnt.y, pnt.z * sd);
};
...
{
    let p3 = createVector((p1.x + p2.x) / 2, (p1.y + p2.y) / 2, (p1.z + p2.z) / 2 * 0.45 * random(0.1, 3));
    move_nearby(p3, sd);
    return p3;
}
...
{
    beginShape();
    vertex(0, current[0].y);
    for (let i = 0; i < current.length; i++)
        vertex(current[i].x, current[i].y);
    vertex(width, current[current.length - 1].y);
    vertex(width, height);
    vertex(0, height);
    endShape(CLOSE);
}
